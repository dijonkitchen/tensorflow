...
absl::StatusOr<std::vector<ArgumentLayoutProperties>>
GetLayoutPropertiesFromAttributes(const mlir::ArrayAttr& all_attrs,
                                  const mlir::ArrayRef<mlir::Type>& types,
                                  size_t num_values) {
  ...
  if (types.size() == 1 && llvm::isa<mlir::TupleType>(types[0])) {
    ...
    return std::vector<ArgumentLayoutProperties>(
        mlir::cast<mlir::TupleType>(types[0]).size());
  }

  // No attributes means everything is default.
  if (!all_attrs) return std::vector<ArgumentLayoutProperties>(num_values);
  ...
  std::vector<ArgumentLayoutProperties> result;
...
constexpr absl::string_view kLayoutModeSuffix = "layout_mode";
constexpr absl::string_view kMemoryKindSuffix = "memory_kind";

absl::StatusOr<std::vector<ArgumentLayoutProperties>> GetLayoutProperties(
    const XlaComputation& computation, size_t num_values,
    absl::string_view prefix) {
  std::vector<ArgumentLayoutProperties> result(num_values);
  ...
  if (auto it = frontend_attrs.find(absl::StrCat(prefix, kLayoutModeSuffix));
    ...
    for (size_t i = 0; i < str_modes.size(); ++i) {
      TF_ASSIGN_OR_RETURN(result[i].mode, LayoutMode::FromString(str_modes[i]));
    }
  }

  if (auto it = frontend_attrs.find(absl::StrCat(prefix, kMemoryKindSuffix));
      it != frontend_attrs.end() && !it->second.empty()) {
    ...
    for (size_t i = 0; i < str_memory_spaces.size(); ++i) {
      ...
      result[i].memory_space = memory_space;
    }
  }
...
absl::StatusOr<std::vector<Shape>> LayoutPropertiesToFlatXlaShapes(
    const std::vector<ArgumentLayoutProperties>& layout_properties,
    const std::vector<Shape>& sharded_shapes,
    const std::vector<Shape>& unsharded_shapes,
    std::function<absl::StatusOr<Shape>(Shape)>
        choose_compact_layout_for_shape_function) {
  if (layout_properties.size() != sharded_shapes.size()) {
    return InvalidArgument(
        "LayoutPropertiesToXlaShapes got mismatched number of sharded shapes "
        "and layout properties (%d vs %d)",
        sharded_shapes.size(), layout_properties.size());
  }
  if (layout_properties.size() != unsharded_shapes.size()) {
    return InvalidArgument(
        "LayoutPropertiesToXlaShapes got mismatched number of unsharded "
        "shapes and layout properties (%d vs %d)",
        unsharded_shapes.size(), layout_properties.size());
  }

  std::vector<Shape> result;
  result.reserve(layout_properties.size());
  for (size_t i = 0; i < layout_properties.size(); ++i) {
    const ArgumentLayoutProperties& layout_prop = layout_properties[i];
