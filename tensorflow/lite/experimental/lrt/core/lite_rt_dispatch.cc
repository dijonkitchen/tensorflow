...
#define INVOKE_FUNC(function, ...)                                       \
  if (!TheApi.interface) {                                               \
    LITE_RT_LOG(LogSeverity::ERROR, "Dispatch API interface not found"); \
    return kLrtStatusErrorRuntimeFailure;                                \
  }                                                                      \
  if (!TheApi.interface->function) {                                     \
    LITE_RT_LOG(LogSeverity::ERROR, #function " not found");             \
    return kLrtStatusErrorRuntimeFailure;                                \
  }                                                                      \
...
#define INVOKE_ASYNC_FUNC(function, ...)                                       \
  if (!TheApi.async_interface) {                                               \
    LITE_RT_LOG(LogSeverity::ERROR, "Dispatch API async interface not found"); \
    return kLrtStatusErrorRuntimeFailure;                                      \
  }                                                                            \
...
#define INVOKE_GRAPH_FUNC(function, ...)                                       \
  if (!TheApi.graph_interface) {                                               \
    LITE_RT_LOG(LogSeverity::ERROR, "Dispatch API graoh interface not found"); \
    return kLrtStatusErrorRuntimeFailure;                                      \
  }                                                                            \
...
LrtStatus LrtDispatchInitialize(const char* shared_lib_path) {
  ...
  if (!lib_handle) {
    LITE_RT_LOG(LogSeverity::ERROR, "Failed to load dispatch library: %s",
                ::dlerror());
    return kLrtStatusErrorRuntimeFailure;
  }
  ...
  if (!LrtDispatchGetApi) {
    LITE_RT_LOG(LogSeverity::ERROR, "LrtDispatchGetApi not found");
    return kLrtStatusErrorRuntimeFailure;
  }
...
LrtStatus LrtDispatchGetApiVersion(LrtDispatchApiVersion* api_version) {
  if (!api_version) {
    LITE_RT_LOG(LogSeverity::ERROR, "Null input");
...
LrtStatus LrtDispatchAttachOutput(
  ...
  if (!TheApi.interface->attach_output) {
    LITE_RT_LOG(LogSeverity::ERROR, "attach_output_tensor_buffer not found");
    return kLrtStatusErrorRuntimeFailure;
  }
...
LrtStatus LrtDispatchLoadExecutable(LrtDispatchDeviceContext device_context,
  ...
  if (!TheApi.graph_interface) {
    LITE_RT_LOG(LogSeverity::ERROR, "Dispatch API graph interface not found");
    return kLrtStatusErrorRuntimeFailure;
  }
  if (!TheApi.graph_interface->load_executable) {
    LITE_RT_LOG(LogSeverity::ERROR, "load_executable not found");
    return kLrtStatusErrorRuntimeFailure;
  }
